...
void handle_forward_user_default_msg(uint32 msg_id, gabriel::base::DB_Handler *handler, gabriel::protocol::server::Forward_User_Msg &forward_msg)
{
    ...
    {
        PARSE_FROM_STRING(Register_Account, msg, forward_msg.msg_data());
        mysqlpp::Query query = handler->query("select * from account where account = %0q");
        query.parse();
        
        if(mysqlpp::StoreQueryResult res = query.store(msg.account()))
        {
...
void handle_forward_user_msg(gabriel::base::DB_Handler *handler, DB_Task *task)
{
    ...
    if(msg.msg_type() == gabriel::protocol::client::DEFAULT_MSG_TYPE)
    {
        handle_forward_user_default_msg(msg.msg_id(), handler, msg);
    }
